...
const PaymentModal = ({ isOpen, onClose, onSave, installment, themeColor = 'blue' }) => {
  ...
  useEffect(() => {
    if (installment) {
      ...
      setFormData({
        actual_paid: remaining.toString(),
        discount: ''
      });
    ...
    try {
      ...
      if (newRemaining <= 0 && discount > 0) {
        const confirmed = window.confirm(
          `سيتم إغلاق العقد بعد هذا الدفع بخصم ${roundAmount(discount).toLocaleString('en-US')} ر.س. هل تريد المتابعة؟`
        );
        if (!confirmed) {
          setLoading(false);
          return;
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div className="bg-slate-800 rounded-2xl w-full max-w-md overflow-hidden max-h-[90dvh] flex flex-col">
        ...
        <div className={`${themeColor === 'indigo' ? 'bg-indigo-900/50' : 'bg-slate-700'} px-6 py-4 flex justify-between items-center shrink-0`}>
          <div>
            <h3 className="text-lg font-bold text-white">تسديد قسط</h3>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-4">
          ...
          <div className="bg-slate-700/50 rounded-xl p-4 border border-slate-600">
            ...
            <div className="flex justify-between mb-2">
              ...
              <span className="text-rose-400 font-bold">{installmentRemaining.toLocaleString('en-US')} ر.س</span>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {/* Actual Paid */}
            <div>
              <label className="block text-sm text-slate-400 mb-2">المبلغ المدفوع</label>
              <input
                type="number"
                step="0.01"
                ...
                required
              />
              <p className="text-xs text-slate-500 mt-1">المبلغ المستلم فعلياً من العميل</p>
            </div>
            ...
            {excessAmount > 0 && (
              <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-3">
                <p className="text-blue-400 text-sm">
                  💡 سيتم توزيع الفائض ({excessAmount.toLocaleString('en-US')} ر.س) تلقائياً على الأقساط التالية
                </p>
              </div>
            )}
            ...
            {shortageAmount > 0 && (
              <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-3">
                <p className="text-amber-400 text-sm">
                  ⚠️ سيتم ترحيل المبلغ المتبقي ({shortageAmount.toLocaleString('en-US')} ر.س) تلقائياً إلى القسط التالي
                </p>
              </div>
            )}
            ...
            <div className="bg-slate-700/30 rounded-xl p-4 border border-slate-600">
              ...
              <p className={`text-2xl font-bold ${newRemaining <= 0 ? 'text-emerald-400' : 'text-white'}`}>
                {Math.max(0, roundAmount(newRemaining)).toLocaleString('en-US')} ر.س
